...
At this point, installation and environment configuration are complete. The
build converts the cloned source code into the runnable Perftest application.
It does not install another product.

The recommended Windows build creates these entry points:

| Path | Purpose |
| --- | --- |
| `srcCs\bin\Release\net8.0\perftest_cs.exe` | The compiled Windows application host |
| `srcCs\bin\Release\net8.0\perftest_cs.dll` | The compiled managed application |
| `bin\release\perftest_cs.bat` | A convenience script generated by `build.bat` |

The convenience script is not a separate application or installation step. It
runs the already-built C# project with `dotnet run --no-build` and passes its
...
that its command-line interface is available:

```bat
bin\release\perftest_cs.bat -help
```
